...
func extractArchive(archive, destination string) error { f, err := os.Open(archive); if err != nil { return err }; defer f.Close(); gz, err := gzip.NewReader(f); if err != nil { return err }; defer gz.Close(); tr := tar.NewReader(gz); for { h, err := tr.Next(); if errors.Is(err, io.EOF) { return nil }; if err != nil { return err }; if !safeArchivePath(h.Name) { return errors.New("unsafe archive path") }; target := filepath.Join(destination, filepath.Clean(h.Name)); if !strings.HasPrefix(target, filepath.Clean(destination)+string(os.PathSeparator)) { return errors.New("archive escapes staging directory") }; if h.FileInfo().IsDir() { if err = os.MkdirAll(target, 0700); err != nil { return err }; continue }; if err = os.MkdirAll(filepath.Dir(target), 0700); err != nil { return err }; dst, err := os.OpenFile(target, os.O_WRONLY|os.O_CREATE|os.O_TRUNC, 0600); if err != nil { return err }; _, err = io.Copy(dst, io.LimitReader(tr, 2<<30)); dst.Close(); if err != nil { return err } } }
...
func safeArchivePath(path string) bool { clean := filepath.Clean(path); return path != "" && clean != "." && clean != ".." && !strings.HasPrefix(clean, "../") && !filepath.IsAbs(path) }
