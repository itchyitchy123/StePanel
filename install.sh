...
install -d -m 0750 "$APP_DIR" "$DATA_DIR/imports"; id "$APP_USER" >/dev/null 2>&1 || useradd --system --home-dir "$APP_DIR" --shell /usr/sbin/nologin "$APP_USER"
...
chown -R "$APP_USER:$APP_USER" "$APP_DIR" "$DATA_DIR"; printf 'STEPANEL_LISTEN=127.0.0.1:8090\nSTEPANEL_IMPORT_ROOT=%s/imports\nSTEPANEL_WEB_ROOT=/var/www\n' "$DATA_DIR" > "$ENV_FILE"; chmod 0600 "$ENV_FILE"
